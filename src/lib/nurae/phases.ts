...
/** asymmetric light pulse — fast attack, gentle eased release (flash feel).
 *  A symmetric bell at 60fps renders only 2-3 partial frames, which reads as a
 *  glitch pop; the shaped release lets the light bloom and decay gracefully. */
export const pulse = (t: number, at: number, attack: number, release: number) => {
  if (t <= at - attack || t >= at + release) return 0
  if (t <= at) return sm(t, at - attack, at)
  return 1 - easeInOutCubic(clamp01((t - at) / release))
}
