...
import { PHASES, sm, bell, clamp01, easeOutCubic, easeInOutCubic } from './phases'
...
export class NuraeEngine {
  ...
  private loop = (now: number) => {
    ...
    const t = this.t
    const flash = bell(t, 6.78, 0.05) + bell(t, 4.32, 0.07) * 0.16
    ...
    const fade = sm(t, 0.0, 0.45) * (1 - sm(t, 11.35, 11.92))
    this.cb.onTick({
      t,
  ...
  private update(t: number, dt: number) {
    ...
    this.beamMat.uniforms.uGrow.value = Math.max(clamp01((t - 0.62) / 0.75), sm(t, 7.6, 8.5))
